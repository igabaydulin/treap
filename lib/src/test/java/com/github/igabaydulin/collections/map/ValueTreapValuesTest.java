...
@DisplayName("Testing TreapMap#values")
class ValueTreapValuesTest {
  ...
  void test_empty_value_treap(ValueTreap<Integer, Integer> valueTreap) {
    Collection<Integer> values = valueTreap.values();
  ...
  void test_to_array(ValueTreap<Integer, Integer> valueTreap) {
    ...
    Collection<Integer> values = valueTreap.values();
    Assertions.assertArrayEquals(new Object[] {3, 5, 6, 2, 4, 1}, values.toArray());
  }
  ...
  void test_value_order(ValueTreap<Integer, Integer> valueTreap) {
    ...
    int i = 0;
    for (int value : valueTreap.values()) {
